#!/usr/bin/env python3
"""
Simple RL Parameter Optimizer for Pet Portrait Oil Painting Conversion
...
class RLState:
    ...
    def mutate(self, exploration_rate: float) -> 'RLState':
        ...
        return RLState(
            denoising_strength=max(0.15, min(0.65, 
                self.denoising_strength + random.uniform(-0.05, 0.05) * exploration_rate)),
            cfg_scale=max(2.0, min(8.0, 
                self.cfg_scale + random.uniform(-0.5, 0.5) * exploration_rate)),
            steps=max(20, min(50, 
                self.steps + int(random.uniform(-3, 3) * exploration_rate))),
            controlnet_weight=max(0.3, min(1.0, 
                self.controlnet_weight + random.uniform(-0.1, 0.1) * exploration_rate))
        )
...
class RLParameterOptimizer:
    ...
    def train(self, training_images: List[Path], episodes: int = 20):
        """
        Main RL training loop - reduced episodes for faster testing
        """
        logger.info("=" * 60)
        ...
        for episode in range(1, episodes + 1):
            ...
            episode_reward = 0.0
            test_subset = random.sample(training_images, min(2, len(training_images)))
...
def main():
    ...
    # Get training images
    training_images = []
    training_images.extend(sorted(input_dir.glob("cat_*.jpg"))[:5])  # Reduced for faster testing
    training_images.extend(sorted(input_dir.glob("dog_*.jpg"))[:5])
    ...
    optimal_state = optimizer.train(training_images, episodes=15)  # Reduced episodes
    ...
    test_images = training_images[-2:]  # Use last 2 images for testing
